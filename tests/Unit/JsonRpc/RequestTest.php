...
test('fromArray throws ProtocolException for invalid jsonrpc version', function () {
    $data = ['jsonrpc' => '1.0', 'id' => 1, 'method' => 'test.method'];
    expect(fn () => Request::fromArray($data))->toThrow(ProtocolException::class);
});

test('fromArray throws ProtocolException for missing jsonrpc', function () {
    $data = ['id' => 1, 'method' => 'test.method'];
    expect(fn () => Request::fromArray($data))->toThrow(ProtocolException::class);
});
...
test('fromArray throws ProtocolException for non-string method', function () {
    $data = ['jsonrpc' => '2.0', 'id' => 1, 'method' => 123];
    expect(fn () => Request::fromArray($data))->toThrow(ProtocolException::class);
});

test('fromArray throws ProtocolException for missing id', function () {
    $data = ['jsonrpc' => '2.0', 'method' => 'test.method'];
    expect(fn () => Request::fromArray($data))->toThrow(ProtocolException::class);
});

test('fromArray throws ProtocolException for non-array params', function () {
    $data = ['jsonrpc' => '2.0', 'id' => 1, 'method' => 'test.method', 'params' => 'invalid'];
